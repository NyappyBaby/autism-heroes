...
default:
    ...
    $query = $db->prepare('SELECT (ami_from + ami_to - :id) AS ami_id, ami_date, membre_pseudo
    FROM forum_amis
    ...
    while ($data = $query->fetch())
    {
        ...
        <td>'.date('d/m/Y',$data['ami_date']).'</td>
        <td><a href="./messagesprives.php?action=repondre&amp;id='.$data['ami_id'].'">Envoyer un MP</a><br />
